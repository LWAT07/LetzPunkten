...
import FileSaving.Data;
import FileSaving.FileChooser;
import FileSaving.FileIo;
import FileSaving.SemData;
import Main.ClassFileEditor.NewClassData;
import Main.Subject.SubjectFrame;
...
public class MainFrame extends JFrame{
    ...
    public MainFrame(){
        ...
        this.setLayout(null);
        this.setBounds(data.getX()+100,data.getY()+100,1500,700); // default size
        if(data.getFullScreen() != 6)
        ...
        for(int y=0; y<Y_COUNTER; y++){
            for(int x=0; x<X_COUNTER; x++){
                if(i>=data.subjectArrayList.size())
                    break;
                SubjectField tempField = new SubjectField(30);
                ...
                tempField.setBorder(new RoundedBorder(20));//(BorderFactory.createLineBorder(Color.black));
                this.add(tempField);
                i++;
            }    
        }
        ...
        //checks if the given bounds from the properties File even fit on the screen
        ...
        //From Stack Overflow
            ...
            for(GraphicsDevice curGs : gs){
                ...
                for(GraphicsConfiguration curGc : gc){
                    ...
                    if(isInside(bounds, this.getBounds()))
                        doesFit = true;
                }
            }
        if(!doesFit){
    ...
    public void loop(){
        ...
        classNameLabel.setLocation(topBorder.getX()+(int)(0.02*data.getWidth()), (topBorder.getHeight()-classNameLabel.getHeight())/2);
        classNameLabel.setSize((sem1Button.getX()-classNameLabel.getX()) - (int)(0.02*data.getWidth()), (int)(topBorder.getHeight()*0.6));
        //makes font smaller when width smaller
        if(classNameLabel.getHeight()<classNameLabel.getWidth()*0.15)
            classNameLabel.setFont(new Font("Arial" , Font.BOLD, (int)(classNameLabel.getHeight())));
        else 
            classNameLabel.setFont(new Font("Arial" , Font.BOLD, (int)(classNameLabel.getWidth()*0.15)));
        ...
        if(isOptionFrame){
            ...
            classFileChooser.setFont(new Font("Arial", Font.BOLD, classFileChooser.getWidth()/10));
    ...
    private boolean isInside(Rectangle bigRectangle, Rectangle smallRectangle){
        return
            bigRectangle.getX() <= smallRectangle.getX()+10 &&
            bigRectangle.getY() <= smallRectangle.getY()+10 &&
            bigRectangle.getX() + bigRectangle.getWidth() + 10 >= smallRectangle.getX() + smallRectangle.getWidth() &&
            bigRectangle.getY() + bigRectangle.getHeight() + 10 >= smallRectangle.getY() + smallRectangle.getHeight();
    }
