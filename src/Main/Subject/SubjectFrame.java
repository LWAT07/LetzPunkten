...
public final class SubjectFrame extends JDialog{
    ...
    public SubjectFrame(int subjectNumber, JFrame parent){
        ...
        // Button panel and scroll pane
        buttonPanel.setBounds(0, 240, this.getWidth(), this.getHeight() - 280);
        buttonPanel.setLayout(null);
        buttonPanel.setPreferredSize(new Dimension(this.getWidth(), 0));
        updateExamLabel();
        buttonPanel.setPreferredSize(new Dimension(this.getWidth(), examArrayList.size()*50));

        scrollPane = new JScrollPane(buttonPanel);
        buttonPanel.setPreferredSize(new Dimension(this.getWidth(), examArrayList.size()*50));
        scrollPane.setBorder(null);
        scrollPane.setBounds(0, 240, this.getWidth()-20, this.getHeight() - 280);
        scrollPane.setHorizontalScrollBar(null);
        this.add(scrollPane);

        //if outside of display changes bounds
        if(this.getX()<0){
    ...
    public void updateExamLabel(){
        ...
        if(examCount != examArrayList.size()){
            for(int j=0; j<examCount; j++){
                ...
                buttonPanel.add(examArrayList.get(j));
            } 
        }
        ...
        if(buttonPanel .getPreferredSize().getHeight() < buttonPanel.getHeight())
            for(ExamLabel ex: examArrayList)
                ex.setSize(462, ex.getHeight());
    }
